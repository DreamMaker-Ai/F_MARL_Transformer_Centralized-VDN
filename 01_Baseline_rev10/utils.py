...
def make_test_results_graph_of_increase_number(agent_type, parent_dir):
    num_red_win_list = []
    num_blue_win_list = []
    num_no_contest_list = []
    ...
    with open(parent_dir + 'nominal_test/result.json') as f:
        ...
        team_rewards_list.append(json_data['episode_team_reward'])
        episode_lens_list.append(json_data['episode_lens'])

    # agent_type = 'platoons', 'companies'
    parent_dir = parent_dir + 'robustness_to_' + agent_type + '/'
    file_dir = ['(10,20)', '(20,30)', '(30,40)', '(40,50)']
    ...
    plt.title('Survive agents ratio, when increase ' + agent_type)
    plt.xlabel('num (bin range center) of ' + agent_type)
    plt.ylabel('survive agents ratio')
    plt.ylim(-0.05, 1.05)
    plt.minorticks_on()
    plt.legend()
    plt.grid()
    ...
    plt.title('Remaining effective force ratio, when increase ' + agent_type)
    plt.xlabel('num (bin range center) of ' + agent_type)
    plt.ylabel('remaining force ratio')
    plt.ylim(-0.05, 1.05)
    plt.minorticks_on()
    plt.legend()
    plt.grid()
    ...
    plt.plot(x, team_rewards_list, color='r', marker='o', label='average team return')
    plt.plot(x, episode_rewards_list, color='r', marker='o', linestyle='dotted',
             label='average episode return')
    plt.plot(x, episode_lens_list, color='b', marker='s', label='average episode length')
    plt.title('Average returns and length of episodes, when increase ' + agent_type)
    plt.xlabel('num (bin range center) of ' + agent_type)
