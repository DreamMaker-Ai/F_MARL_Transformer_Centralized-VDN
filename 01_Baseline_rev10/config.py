...
class Config:
    def __init__(self):
        ...
        # self.model_dir = 'models/model_10200/'  # newest file -> 'ls -ltr'
        self.model_dir = None

        if self.model_dir:  # starting steps for continual training
            self.n0 = 10230  # learner update cycles. Should be read from tensorboard
            self.actor_cycles = 141530  # Should be read from tensorboard
        else:
            self.n0 = 0
            self.actor_cycles = 0
        ...
        self.gamma = 0.96
        self.max_steps = 150  # Default = 150 for training, 200 for test
