...
from battlefield_strategy_rev10_finetuning import BattleFieldStrategy
...
class Actor:
    ...
    def rollout(self, current_weights):
        ...
        if self.env.config.prioritized_replay:
            ...
            states = \
                np.vstack([transition[0] for transition in self.buffer])  # (b,n,g,g,h*n_frames)
            actions = np.vstack([transition[1] for transition in self.buffer])  # (b,n)
            rewards = np.array([transition[2] for transition in self.buffer])  # (b,)
            next_states = \
                np.vstack([transition[3] for transition in self.buffer])  # (b,n,g,g,h*n_frames)
            dones = np.array([transition[4] for transition in self.buffer])  # (b,)
            mask = np.vstack([transition[5] for transition in self.buffer])  # (b,n)
            next_states_for_q = \
                np.vstack([transition[7] for transition in self.buffer])  # (b,n,g,g,h*n_frames)

            next_q_values, _ = \
                self.policy(next_states_for_q, mask, training=False)  # (b,n,action_dim)
            next_actions = tf.argmax(next_q_values, axis=-1)  # (b,n)
            next_actions = tf.cast(next_actions, dtype=tf.int32)
            next_actions_one_hot = \
                tf.one_hot(next_actions, depth=self.action_space_dim)  # (b,n,action_dim)

            next_maxQ = tf.reduce_sum(next_q_values * next_actions_one_hot, axis=-1)  # (b,n)

            next_maxQ = next_maxQ * mask.astype(np.float32)  # (b,n)
            next_maxQ = tf.reduce_sum(next_maxQ, axis=1)  # (b,)

            TQ = rewards + self.gamma * (1 - dones) * next_maxQ  # (b,)

            q_values, _ = self.policy(states, mask, training=False)  # (b,n,action_dim)
            actions_one_hot = tf.one_hot(actions, depth=self.action_space_dim)  # (b,n,action_dim)
            Q = tf.reduce_sum(q_values * actions_one_hot, axis=-1)  # (b,n)

            Q = Q * mask.astype(np.float32)  # (b,n)
            Q = tf.reduce_sum(Q, axis=1)  # (b,)

            masked_td_errors = np.abs(TQ - Q)  # (b,)

            masked_td_errors = \
                masked_td_errors / np.sum(mask.astype(np.float32), axis=-1)  # (b,)
