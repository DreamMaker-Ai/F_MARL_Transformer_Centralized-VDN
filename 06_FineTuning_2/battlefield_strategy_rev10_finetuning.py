...
class BattleFieldStrategy(gym.Env):
    ...
    def step(self, actions):
        ...
        alive_reds = count_alive_agents(self.reds)

        reward += self.enemy_attrition_reward_1(
            blues_force_after, blues_force_before, self.initial_effective_blues_force,
            reds_force_after, reds_force_before, self.initial_effective_reds_force,
            len(x1), x1, y1, alive_reds
        )
        reward += self.enemy_attrition_reward_2(blues_num_after, blues_num_before)

        """ Reward to engaing agents """
        # reward += self.engaging_agents_reward(x1, y1)
    ...
    def enemy_attrition_reward_1(self, blues_force_after, blues_force_before, B0,
                                 reds_force_after, reds_force_before, R0,
                                 num_engage, x1, y1, alive_reds):
        c2 = 2.0
        reward = 0.0

        # reward only when engagement occurs.
        if (blues_force_after != blues_force_before) and (reds_force_after != reds_force_before):

            num_engaging_agents = self.count_engaging_agents(x1, y1)
            # r = 0.1 / self.config.max_num_red_agents * num_engaging_agents
            if alive_reds > 0:
                r = 0.5 / alive_reds * num_engaging_agents
            else:
                r = 0.0
    ...
    def enemy_attrition_reward_2(self, blues_num_after, blues_num_before):
        c2 = 2.0
    ...
    def count_engaging_agents(self, x1, y1):
        counter = 0
    ...
    def engaging_agents_reward(self, x1, y1):  # not used
        c = 0.2 / self.config.max_num_red_agents

        if len(x1) > 0:
            num_engaging_agents = self.count_engaging_agents(x1, y1)
        else:
            num_engaging_agents = 0

        reward = c * num_engaging_agents
